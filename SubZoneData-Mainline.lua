-- Grab local references to global variables. We are trading RAM to decrease CPU usage and hopefully increase FPS
local GetAreaInfo = C_Map.GetAreaInfo
local LibStub = LibStub
local UnitClassBase = UnitClassBase
local UnitFactionGroup = UnitFactionGroup

------------------- Get addon reference --------------------
---@type RepByZone
local RepByZone = LibStub("AceAddon-3.0"):GetAddon("RepByZone")
local playerClass = UnitClassBase("player")

---@return table<string, number?>
function RepByZone:SubZonesAndFactionsList()
	local H = UnitFactionGroup("player") == "Horde"
	local A = UnitFactionGroup("player") == "Alliance"

	---@param allianceFactionID number
	---@param hordeFactionID number
	---@return number?
	local function GetFactionID(allianceFactionID, hordeFactionID)
		if A then
			return allianceFactionID
		elseif H then
			return hordeFactionID
		end
	end

	---@param faction "Alliance"|"Horde"
	---@param factionID number
	---@return number?
	local function GetSingleFactionID(faction, factionID)
		if faction == "Alliance" and A then
			return factionID
		elseif faction == "Horde" and H then
			return factionID
		end
	end

	---@param faction "Alliance"|"Horde"
	---@param class string
	---@param factionID number
	---@return number?
	local function GetClassFactionID(faction, class, factionID)
		if playerClass == class then
			return GetSingleFactionID(faction, factionID)
		end
	end

	---@type table<string, number?>
	local subZonesAndFactions = {
		-- [GetAreaInfo(areaID)]	= factionID
		-- see https://wago.tools/db2/AreaTable

		[GetAreaInfo(35)]		= 21,						-- Booty Bay/Booty Bay
		[GetAreaInfo(36)]		= GetFactionID(730, 729),	-- Alterac Mountains/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(43)]		= 87,						-- Wild Shore/Bloodsail Buccaneers
		[GetAreaInfo(100)]		= 47,						-- Nesingwary's Expedition/Ironforge
		[GetAreaInfo(133)]		= 54,						-- New Tinkertown/Gnomeregan
		[GetAreaInfo(150)]		= 72,						-- Menethil Harbor/Stormwind
		[GetAreaInfo(152)]		= 1106,						-- The Bulwark/Argent Crusade
		[GetAreaInfo(193)]		= GetFactionID(72, 68),		-- Ruins of Andorhal/Stormwind or Undercity
		[GetAreaInfo(196)]		= 72,						-- Uthor's Tomb/Stormwind
		[GetAreaInfo(197)]		= 72,						-- Sorrow Hill/Stormwind
		[GetAreaInfo(199)]		= 72,						-- Felstone Field/Stormwind
		[GetAreaInfo(201)]		= 68,						-- Gahrron's Withering/Undercity
		[GetAreaInfo(202)]		= 68,						-- Writhing Haunt/Undercity
		[GetAreaInfo(204)]		= 1134,						-- Pyrewood Village/Gilneas
		[GetAreaInfo(233)]		= 1134,						-- Ambermill/Gilneas
		[GetAreaInfo(250)]		= 59,						-- Ruins of Thaurissan/Thorium Brotherhood
		[GetAreaInfo(279)]		= 1090,						-- Dalaran Crater/Kirin Tor
		[GetAreaInfo(280)]		= 349,						-- Strahnbrad/Ravenholdt
		[GetAreaInfo(288)]		= 72,						-- Azurelode Mine/Stormwind
		[GetAreaInfo(297)]		= 81,						-- Jaguero Isle/Thunder Bluff
		[GetAreaInfo(299)]		= 72,						-- Menethil Bay/Stormwind
		[GetAreaInfo(313)]		= 349,						-- Northfold Manor/Ravenholdt
		[GetAreaInfo(315)]		= 72,						-- Dabyrie's Farmstead/Stormwind
		[GetAreaInfo(317)]		= GetFactionID(1174, 530),	-- Witherbark Village/Wildhammer Clan or Darkspear Trolls
		[GetAreaInfo(320)]		= 72,						-- Refuge Pointe/Stormwind
		[GetAreaInfo(321)]		= 68,						-- Hammerfall/Undercity
		[GetAreaInfo(324)]		= 349,						-- Stromgarde Keep/Ravenholdt
		[GetAreaInfo(327)]		= 21,						-- Faldir's Cove/Booty Bay
		[GetAreaInfo(328)]		= 21,						-- The Drowned Reef/Booty Bay
		[GetAreaInfo(330)]		= 47,						-- Thandol Span/Ironforge
		[GetAreaInfo(350)]		= 69,						-- Quel'Danil Lodge/Darnassus
		[GetAreaInfo(359)]		= GetFactionID(47, 81),		-- Bael Modan/Ironforge or Thunder Bluff
		[GetAreaInfo(367)]		= 530,						-- Sen'jen Village/Darkspear Trolls
		[GetAreaInfo(368)]		= 530,						-- Echo Isles/Darkspear Trolls
		[GetAreaInfo(385)]		= 72,						-- Northwatch Hold/Stormwind
		[GetAreaInfo(392)]		= 470,						-- Ratchet/Ratchet
		[GetAreaInfo(393)]		= 530,						-- Darkspear Strand/Darkspear Trolls
		[GetAreaInfo(401)]		= GetClassFactionID("Horde", "SHAMAN", 1135), -- The Tidus Stair/The Earthen Ring (only for Horde shamans)
		[GetAreaInfo(439)]		= GetFactionID(54, 76),		-- The Shimmering Flats/Gnomeregan or Orgrimmar
		[GetAreaInfo(484)]		= GetFactionID(69, 81),		-- Freewind Post/Darnassus or Thunder Bluff
		[GetAreaInfo(501)]		= 369,						-- Beezil's Wreck/Gadgetzan
		[GetAreaInfo(517)]		= 1358,						-- Tidefury Cove/Nat Pagle
		[GetAreaInfo(596)]		= 470,						-- Kodo Graveyard/Ratchet
		[GetAreaInfo(603)]		= 69,						-- Sargeron/Darnassus
		[GetAreaInfo(604)]		= 93,						-- Magram Village (Shok'Thokar)/Magram Clan Centaur
		[GetAreaInfo(606)]		= 92,						-- Gelkis Village/Gelkis Clan Centaur
		[GetAreaInfo(608)]		= 69,						-- Nijel's Point/Darnassus
		[GetAreaInfo(609)]		= 93,						-- Kolkar Village/Magram Clan Centaur
		[GetAreaInfo(657)]		= 930,						-- The Harborage/Exodar
		[GetAreaInfo(702)]		= 69,						-- Rut'theran Village/Darnassus
		[GetAreaInfo(721)]		= 54,						-- Gnomeregan/Gnomeregan
		[GetAreaInfo(813)]		= 1106,						-- The Bulwark/Argent Crusade
		[GetAreaInfo(880)]		= 1174,						-- Thandol Span (Arathi Highlands)/Wildhammer Clan
		[GetAreaInfo(881)]		= 47,						-- Thandol Span (Wetlands)/Ironforge
		[GetAreaInfo(896)]		= GetFactionID(730, 729),	-- Purgation Isle/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(978)]		= GetFactionID(1174, 530),	-- Zul'Farrak/Wildhammer Clan or Darkspear Trolls
		[GetAreaInfo(987)]		= 1133,						-- Land's End Beach/Bilgewater Cartel
		[GetAreaInfo(989)]		= 1173,						-- Ruins of Uldum/Ramkahen
		[GetAreaInfo(990)]		= 1173,						-- Valley of the Watchers/Ramkahen
		[GetAreaInfo(1016)]		= 69,						-- Direforge Hill/Darnassus
		[GetAreaInfo(1025)]		= 69,						-- The Green Belt/Darnassus
		[GetAreaInfo(1057)]		= GetFactionID(47, 68),		-- Thoradin's Wall (Hillsbrad Foothills)/Ironforge or Undercity
		[GetAreaInfo(1216)]		= 579,						-- Timbermaw Hold/Timbermaw Hold
		[GetAreaInfo(1220)]		= 69,						-- Darnassian Base Camp/Darnassus
		[GetAreaInfo(1336)]		= GetFactionID(54, 1133),	-- Lost Rigger Cove/Gnomeregan or Bilgewater Cartel
		[GetAreaInfo(1446)]		= 59,						-- Thorium Point/Thorium Brotherhood
		[GetAreaInfo(1658)]		= 609,						-- Cenarion Enclave/Cenarion Circle
		[GetAreaInfo(1677)]		= GetFactionID(730, 729),	-- Gavin's Naze/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(1678)]		= 72,						-- Sofera's Naze/Stormwind
		[GetAreaInfo(1679)]		= GetFactionID(730, 729),	-- Corrahn's Dagger/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(1680)]		= GetFactionID(730, 729),	-- The Headland/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(1739)]		= 1133,						-- Hardwrench Hideaway/Bilgewater Cartel
		[GetAreaInfo(1741)]		= 87,						-- Gurubashi Arena/Bloodsail Buccaneers
		[GetAreaInfo(1757)]		= 87,						-- The Crimson Veil/Bloodsail Buccaneers
		[GetAreaInfo(1758)]		= 87,						-- The Riptide/Bloodsail Buccaneers
		[GetAreaInfo(1759)]		= 87,						-- The Damsel's Luck/Bloodsail Buccaneers
		[GetAreaInfo(1761)]		= 579,						-- Deadwood Village/Timbermaw Hold
		[GetAreaInfo(1762)]		= 579,						-- Felpaw Village/Timbermaw Hold
		[GetAreaInfo(1769)]		= 579,						-- Timbermaw Hold/Timbermaw Hold
		[GetAreaInfo(1778)]		= 369,						-- Sorrowmurk/Gadgetzan
		[GetAreaInfo(1797)]		= 1133,						-- Stagalbog/Bilgewater Cartel
		[GetAreaInfo(1837)]		= GetFactionID(1174, 530),	-- Witherbark Caverns/Wildhammer Clan or Darkspear Trolls
		[GetAreaInfo(1857)]		= GetFactionID(47, 68),		-- Thoradin's Wall (Arathi Highlands)/Ironforge or Undercity
		[GetAreaInfo(1858)]		= 1174,						-- Boulder'gor/Wildhammer Clan
		[GetAreaInfo(1941)]		= 989,						-- Caverns of Time/Keepers of Time
		[GetAreaInfo(1998)]		= 1134,						-- Talonbranch Glade/Gilneas
		[GetAreaInfo(2079)]		= GetFactionID(54, 68),		-- Alcaz Island/Gnomeregan or Undercity
		[GetAreaInfo(2097)]		= GetFactionID(69, 81),		-- Darkcloud Pinnacle/Darnassus or Thunder Bluff
		[GetAreaInfo(2157)]		= GetFactionID(47, 81),		-- Bael'dun Keep/Ironforge or Thunder Bluff
		[GetAreaInfo(2177)]		= 87,						-- Battle Ring/Bloodsail Buccaneers
		[GetAreaInfo(2240)]		= GetFactionID(54, 1133),	-- Mirage Raceway/Gnomeregan or Bilgewater Cartel
		[GetAreaInfo(2241)]		= 589,						-- Frostsaber Rock/Wintersaber Trainers
		[GetAreaInfo(2242)]		= 69,						-- The Hidden Grove/Darnassus
		[GetAreaInfo(2243)]		= 579,						-- Timbermaw Post/Timbermaw Hold
		[GetAreaInfo(2244)]		= 579,						-- Winterfall Village/Timbermaw Hold
		[GetAreaInfo(2246)]		= 579,						-- Frostfire Hot Springs/Timbermaw Hold
		[GetAreaInfo(2248)]		= 47,						-- Dun Mandarr/Ironforge
		[GetAreaInfo(2253)]		= 69,						-- Starfall Village/Darnassus
		[GetAreaInfo(2257)]		= 72,						-- Deeprun Tram/Stormwind
		[GetAreaInfo(2276)]		= 69,						-- Quel'Lithien Lodge/Darnassus
		[GetAreaInfo(2300)]		= 989,						-- Caverns of Time/Keepers of Time
		[GetAreaInfo(2379)]		= 72,						-- Azurelode Mine/Stormwind
		[GetAreaInfo(2404)]		= 69,						-- Tethris Aran/Darnassus
		[GetAreaInfo(2406)]		= 69,						-- Ranazjar Isle/Darnassus
		[GetAreaInfo(2407)]		= 470,						-- Kormek's Hut/Ratchet
		[GetAreaInfo(2408)]		= 530,						-- Shadowprey Village/Darkspear Trolls
		[GetAreaInfo(2539)]		= 530,						-- Malaka'jin/Darkspear Trolls
		[GetAreaInfo(2597)]		= GetFactionID(730, 729),	-- Alterac Valley/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(2617)]		= 470,						-- Scrabblescrew's Camp/Ratchet
		[GetAreaInfo(2738)]		= 69,						-- Southwind Village/Darnassus
		[GetAreaInfo(2839)]		= GetFactionID(730, 729),	-- Alterac Valley/Stormpike Guard or Frostwolf Clan
		[GetAreaInfo(2897)]		= 530,						-- Zoram'gar Outpost/Darkspear Trolls
		[GetAreaInfo(3137)]		= 69,						-- Talrendis Point/Darnassus
		[GetAreaInfo(3197)]		= 72,						-- Chillwind Camp/Stormwind
		[GetAreaInfo(3319)]		= 890,						-- Silverwing Grove/Silverwing Sentinels
		[GetAreaInfo(3426)]		= 69,						-- Staghelm Point/Darnassus
		[GetAreaInfo(3427)]		= 47,						-- Bronzebeard Encampment/Ironforge
		[GetAreaInfo(3486)]		= 349,						-- Ravenholdt Manor/Ravenholdt
		[GetAreaInfo(4010)]		= 369,						-- Mudsprocket/Gadgetzan
		[GetAreaInfo(4690)]		= 1135,						-- Thunder Peak/The Earthen Ring
		[GetAreaInfo(4715)]		= 1133,						-- The Skunkworks/Bilgewater Cartel
		[GetAreaInfo(4745)]		= 76,						-- Orgrimmar Rear Gate/Orgrimmar
		[GetAreaInfo(4798)]		= 1068,						-- Thoargad's Camp/Explorer's League
		[GetAreaInfo(4803)]		= 911,						-- Furien's Post/Silvermoon City
		[GetAreaInfo(4882)]		= 369,						-- Marshal's Stand/Gadgetzan
		[GetAreaInfo(4883)]		= 369,						-- Mossy Pile/Gadgetzan
		[GetAreaInfo(4925)]		= 609,						-- Thunk's Abode/Cenarion Circle
		[GetAreaInfo(4927)]		= 76,						-- The Fold/Orgrimmar
		[GetAreaInfo(4933)]		= 76,						-- Krom'gar Fortress/Orgrimmar
		[GetAreaInfo(4934)]		= 1133,						-- The Sludgewerks/Bilgewater Cartel
		[GetAreaInfo(4940)]		= 1068,						-- Northwatch Expedition Base Camp/Explorer's League
		[GetAreaInfo(5011)]		= 81,						-- Westreach Summit/Thunder Bluff
		[GetAreaInfo(5028)]		= 87,						-- Southsea Holdfast/Bloodsail Buccaneers
		[GetAreaInfo(5041)]		= GetFactionID(54, 1133),	-- Fizzle & Pozzik's Speedbarge/Gnomeregan or Bilgewater Cartel
		[GetAreaInfo(5044)]		= 609,						-- The Tainted Forest/Cenarion Circle
		[GetAreaInfo(5048)]		= GetFactionID(54, 1133),	-- Raceway Ruins/Gnomeregan or Bilgewater Cartel
		[GetAreaInfo(5083)]		= 911,						-- Sunveil Excursion/Silvermoon City
		[GetAreaInfo(5117)]		= 1133,						-- Nozzlepot's Outpost/Bilgewater Cartel
		[GetAreaInfo(5121)]		= 68,						-- Galen's Fall/Undercity
		[GetAreaInfo(5150)]		= 47,						-- Dwarven District/Ironforge
		[GetAreaInfo(5169)]		= 530,						-- Valley of Spirits/Darkspear Trolls
		[GetAreaInfo(5171)]		= 81,						-- Valley of Wisdom/Thunder Bluff
		[GetAreaInfo(5305)]		= 69,						-- Greenwarden's Grove/Darnassus
		[GetAreaInfo(5317)]		= 530,						-- Bambala/Darkspear Trolls
		[GetAreaInfo(5367)]		= 942,						-- The Mender's Stead/Cenarion Circle
		[GetAreaInfo(5388)]		= 911,						-- Dawnrise Expedition/Silvermoon City
		[GetAreaInfo(5458)]		= 369,						-- Bogpaddle/Gadgetzan
		[GetAreaInfo(5476)]		= 1134,						-- Pyrewood Chapel/Gilneas
		[GetAreaInfo(5477)]		= 1134,						-- Pyrewood Inn/Gilneas
		[GetAreaInfo(5478)]		= 1134,						-- Pyrewood Town Hall/Gilneas
		[GetAreaInfo(5480)]		= 1134,						-- Gilneas Liberation Base Camp/Gilneas
		[GetAreaInfo(5495)]		= 54,						-- Gnomeregan/Gnomeregan
		[GetAreaInfo(5496)]		= 369,						-- Fuselight/Gadgetzan
		[GetAreaInfo(5497)]		= 369,						-- Fuselight-by-the-Sea/Gadgetzan
		[GetAreaInfo(5524)]		= 911,						-- Bloodwatcher Point/Silvermoon City
		[GetAreaInfo(5564)]		= 72,						-- Dragon's Mouth/Stormwind
		[GetAreaInfo(5645)]		= 609,						-- Whisperwind Grove/Cenarion Circle
		[GetAreaInfo(5649)]		= 609,						-- Wildheart Point/Cenarion Circle
		[GetAreaInfo(5654)]		= 59,						-- Chiselgrip/Thorium Brotherhood
		[GetAreaInfo(5675)]		= GetFactionID(69, 81),		-- Arikara's Needle/Darnassus or Thunder Bluff
		[GetAreaInfo(5687)]		= 1134,						-- Darnassus: The Howling Oak/Gilneas
		[GetAreaInfo(5705)]		= GetFactionID(47, 530),	-- Snowden Chalet/Ironforge or Darkspear Trolls
		[GetAreaInfo(5706)]		= 369,						-- The Steam Pools/Gadgetzan

		--------- TBC ---------
		[GetAreaInfo(3482)]		= 922,						-- The Dead Scar (Eversong Woods)/Tranquillien
		[GetAreaInfo(3514)]		= 922,						-- The Dead Scar (Ghostlands)/Tranquillien
		[GetAreaInfo(3530)]		= GetFactionID(930, 911),	-- Shadow Ridge/Exodar or Silvermoon City
		[GetAreaInfo(3547)]		= 1077,						-- Throne of Kil'jaeden/Shattered Sun Offensive
		[GetAreaInfo(3552)]		= 978,						-- Temple of Telhamat/Kurenai
		[GetAreaInfo(3554)]		= 911,						-- Falcon Watch/Silvermoon City
		[GetAreaInfo(3555)]		= 941,						-- Mag'har Post/The Mag'har
		[GetAreaInfo(3569)]		= 69,						-- Tides' Hollow/Darnassus
		[GetAreaInfo(3573)]		= 72,						-- Odesyus' Landing/Stormwind
		[GetAreaInfo(3590)]		= 69,						-- Wrathscale Lair/Darnassus
		[GetAreaInfo(3591)]		= 69,						-- Ruins of Loreth'Aran/Darnassus
		[GetAreaInfo(3596)]		= GetClassFactionID("Alliance", "SHAMAN", 1135), -- The Hidden Reef/The Earthen Ring (only for Alliance shamans)
		[GetAreaInfo(3598)]		= 69,						-- Wyrmscar Island/Darnassus
		[GetAreaInfo(3615)]		= 1135,						-- Throne of the Elements/The Earthen Ring
		[GetAreaInfo(3623)]		= 933,						-- Aeris Landing/The Consortium
		[GetAreaInfo(3636)]		= 1135,						-- Elemental Plateau/The Earthen Ring
		[GetAreaInfo(3673)]		= 47,						-- Nesingwary Safari/Ironforge
		[GetAreaInfo(3628)]		= GetFactionID(930, 911),	-- Halaa/Exodar or Silvermoon City
		[GetAreaInfo(3630)]		= 933,						-- Oshu'gun/The Consortium
		[GetAreaInfo(3631)]		= 933,						-- Spirit Fields/The Consortium
		[GetAreaInfo(3644)]		= 930,						-- Telredor/Exodar
		[GetAreaInfo(3645)]		= 530,						-- Zabra'jin/Darkspear Trolls
		[GetAreaInfo(3646)]		= 970,						-- Quagg Ridge/Sporeggar
		[GetAreaInfo(3647)]		= 970,						-- The Spawning Glen/Sporeggar
		[GetAreaInfo(3649)]		= 970,						-- Sporeggar/Sporeggar
		[GetAreaInfo(3652)]		= 970,						-- Funggor Cavern/Sporeggar
		[GetAreaInfo(3674)]		= 942,						-- Cenarion Thicket/Cenarion Expedition
		[GetAreaInfo(3679)]		= 1031,						-- Skettis/Sha'tari Skyguard
		[GetAreaInfo(3680)]		= 1031,						-- Blackwind Valley/Sha'tari Skyguard
		[GetAreaInfo(3681)]		= 934,						-- Firewing Point/The Scryers
		[GetAreaInfo(3683)]		= 941,						-- Stonebreaker Hold/The Mag'har
		[GetAreaInfo(3684)]		= 69,						-- Allerian Stronghold/Darnassus
		[GetAreaInfo(3690)]		= 1031,						-- Blackwind Lake/Sha'tari Skyguard
		[GetAreaInfo(3691)]		= 1031,						-- Lake Ere'Noru/Sha'tari Skyguard
		[GetAreaInfo(3718)]		= 530,						-- Swamprat Post/Darkspear Trolls
		[GetAreaInfo(3719)]		= 941,						-- Bleeding Hollow Ruins/The Mag'har
		[GetAreaInfo(3744)]		= 76,						-- Shadowmoon Village/Orgrimmar
		[GetAreaInfo(3745)]		= 1174,						-- Wildhammer Stronghold/Wildhammer Clan
		[GetAreaInfo(3754)]		= 932,						-- Altar of Sha'tar/The Aldor
		[GetAreaInfo(3758)]		= 1015,						-- Netherwing Fields/Netherwing
		[GetAreaInfo(3759)]		= 1015,						-- Netherwing Ledge/Netherwing
		[GetAreaInfo(3766)]		= 978,						-- Orebor Harborage/Kurenai
		[GetAreaInfo(3778)]		= 1068,						-- Draenethyst Mine/Explorer's League
		[GetAreaInfo(3769)]		= 941,						-- Thunderlord Stronghold/The Mag'har
		[GetAreaInfo(3771)]		= 69,						-- The Living Grove/Darnassus
		[GetAreaInfo(3772)]		= 69,						-- Sylvanaar/Darnassus
		[GetAreaInfo(3784)]		= 1031,						-- Forge Camp: Terror/Sha'tari Skyguard
		[GetAreaInfo(3785)]		= 1031,						-- Forge Camp: Wrath/Sha'tari Skyguard
		[GetAreaInfo(3792)]		= 933,						-- Mana-Tombs/The Consortium
		[GetAreaInfo(3801)]		= 941,						-- Mag'har Grounds/The Mag'har
		[GetAreaInfo(3806)]		= 911,						-- Supply Caravan/Silvermoon City
		[GetAreaInfo(3808)]		= 942,						-- Cenarion Post/Cenarion Expedition
		[GetAreaInfo(3816)]		= 21,						-- Zeppelin Crash/Booty Bay
		[GetAreaInfo(3828)]		= 942,						-- Ruuan Weald/Cenarion Expedition
		[GetAreaInfo(3839)]		= 1011,						-- Abandoned Armory/Lower City
		[GetAreaInfo(3842)]		= 935,						-- Tempest Keep (Netherstorm)/The Sha'tar
		[GetAreaInfo(3864)]		= 1031,						-- Bash'ir Landing/Sha'tari Skyguard
		[GetAreaInfo(3896)]		= 932,						-- Aldor Rise/The Aldor
		[GetAreaInfo(3898)]		= 934,						-- Scryer's Tier/The Scryers
		[GetAreaInfo(3899)]		= 1011,						-- Lower City/Lower City
		[GetAreaInfo(3901)]		= 69,						-- Allerian Post/Darnassus
		[GetAreaInfo(3902)]		= 941,						-- Stonebreaker Camp/The Mag'har
		[GetAreaInfo(3918)]		= 54,						-- Toshley's Station/Gnomeregan
		[GetAreaInfo(3936)]		= 1174,						-- Deathforge Tower/Wildhammer Clan
		[GetAreaInfo(3937)]		= 76,						-- Slag Watch/Orgrimmar
		[GetAreaInfo(3938)]		= 934,						-- Sanctum of the Stars/The Scryers
		[GetAreaInfo(3951)]		= 942,						-- Evergrove/Cenarion Expedition
		[GetAreaInfo(3952)]		= 942,						-- Wyrmskull Bridge/Cenarion Expedition
		[GetAreaInfo(3958)]		= 1031,						-- Sha'tari Base Camp/Sha'tari Skyguard
		[GetAreaInfo(3965)]		= 1015,						-- Netherwing Mines/Netherwing
		[GetAreaInfo(3966)]		= 1015,						-- Dragonmaw Base Camp/Netherwing
		[GetAreaInfo(3964)]		= 1031,						-- Skyguard Outpost/Sha'tari Skyguard
		[GetAreaInfo(3973)]		= 1031,						-- Blackwind Landing/Sha'tari Skyguard
		[GetAreaInfo(3974)]		= 1031,						-- Veil Harr'ik/Sha'tari Skyguard
		[GetAreaInfo(3975)]		= 1031,						-- Terokk's Rest/Sha'tari Skyguard
		[GetAreaInfo(3976)]		= 1031,						-- Veil Ala'rak/Sha'tari Skyguard
		[GetAreaInfo(3977)]		= 1031,						-- Upper Veil Shil'ak/Sha'tari Skyguard
		[GetAreaInfo(3978)]		= 1031,						-- Lower Veil Shil'ak/Sha'tari Skyguard
		[GetAreaInfo(5554)]		= 935,						-- The Mechanar Entrance/The Sha'tar
		[GetAreaInfo(5555)]		= 935,						-- The Botanica Entrance/The Sha'tar
		[GetAreaInfo(5556)]		= 935,						-- The Arcatraz Entrance/The Sha'tar

		--------- WotLK ---------
		[GetAreaInfo(3987)]		= 1073,						-- The Isle of Spears/The Kalu'ak
		[GetAreaInfo(3988)]		= 1073,						-- Kamagua/The Kalu'ak
		[GetAreaInfo(3990)]		= 1073,						-- Scalawag Point/The Kalu'ak
		[GetAreaInfo(3996)]		= 1068,						-- Baelgun's Excavation Site/Explorer's League
		[GetAreaInfo(3997)]		= 1068,						-- Explorers' League Outpost/Explorers' League
		[GetAreaInfo(3999)]		= 1068,						-- Steel Gate/Explorers' League
		[GetAreaInfo(4018)]		= 1064,						-- Camp Winterhoof/The Taunka
		[GetAreaInfo(4023)]		= 1090,						-- Amber Ledge/Kirin Tor
		[GetAreaInfo(4024)]		= 1090,						-- Coldarra/Kirin Tor
		[GetAreaInfo(4033)]		= 942,						-- Winterfin Village/Cenarion Expedition
		[GetAreaInfo(4037)]		= 1064,						-- Taunka'le Village/The Taunka
		[GetAreaInfo(4040)]		= 1073,						-- Njord's Breath Bay/The Kalu'ak
		[GetAreaInfo(4041)]		= 1073,						-- Kaskala/The Kalu'ak
		[GetAreaInfo(4097)]		= 942,						-- Winterfin Caverns/Cenarion Expedition
		[GetAreaInfo(4099)]		= 942,						-- Winterfin Retreat/Cenarion Expedition
		[GetAreaInfo(4113)]		= 1073,						-- Unu'pe/The Kalu'ak
		[GetAreaInfo(4120)]		= 1090,						-- The Nexus/Kirin Tor
		[GetAreaInfo(4121)]		= 1090,						-- Transitus Shield/Kirin Tor
		[GetAreaInfo(4151)]		= 1064,						-- Westwind Refuge Camp/The Taunka
		[GetAreaInfo(4152)]		= 1073,						-- Moa'ki Harbor/The Kalu'ak
		[GetAreaInfo(4155)]		= 1073,						-- The Half Shell/The Kalu'ak
		[GetAreaInfo(4158)]		= 1037,						-- Stars' Rest/Alliance Vanguard
		[GetAreaInfo(4165)]		= 1052,						-- Agmar's Hammer/Horde Expedition
		[GetAreaInfo(4169)]		= 1037,						-- Fordragon Hold/Alliance Vanguard
		[GetAreaInfo(4170)]		= 1052,						-- Kor'kron Vanguard/Horde Expedition
		[GetAreaInfo(4171)]		= GetFactionID(1037, 1052),	-- The Court of Skulls/Alliance Vanguard or Horde Expedition
		[GetAreaInfo(4172)]		= GetFactionID(1037, 1052),	-- Angrathar the Wrathgate/Alliance Vanguard or Horde Expedition
		[GetAreaInfo(4177)]		= 1037,						-- Wintergarde Keep/Alliance Vanguard
		[GetAreaInfo(4178)]		= 1037,						-- Wintergarde Mine/Alliance Vanguarde
		[GetAreaInfo(4186)]		= 1067,						-- Venomspite/The Hand of Vengeance
		[GetAreaInfo(4188)]		= 1037,						-- The Carrion Fields/Alliance Vanguard
		[GetAreaInfo(4189)]		= 942,						-- D.E.H.T.A Encampment/Cenarion Expedition
		[GetAreaInfo(4190)]		= 1037,						-- Thorson's Post/Alliance Vanguard
		[GetAreaInfo(4191)]		= 1106,						-- Light's Trust/Argent Crusade
		[GetAreaInfo(4211)]		= 1064,						-- Camp Oneqwah/The Taunka
		[GetAreaInfo(4224)]		= 1073,						-- The Briny Pinnacle/The Kalu'ak
		[GetAreaInfo(4226)]		= 1073,						-- Iskaal/The Kalu'ak
		[GetAreaInfo(4227)]		= 530,						-- Dragon's Fall/Darkspear Trolls
		[GetAreaInfo(4233)]		= 1106,						-- Dawn's Reach/Argent Crusade
		[GetAreaInfo(4234)]		= 1106,						-- Naxxramas/Argent Crusade
		[GetAreaInfo(4243)]		= 1037,						-- Wintergarde Crypt/Alliance Vanguard
		[GetAreaInfo(4246)]		= 1037,						-- Wintergarde Mausoleum/Alliance Vanguard
		[GetAreaInfo(4256)]		= GetFactionID(1094, 1124),	-- Drak'mar Lake/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4261)]		= 68,						-- Ghostblade Point/Undercity
		[GetAreaInfo(4281)]		= 1098,						-- Acherus: The Ebon Hold (Eastern Kingdoms)/Knights of the Ebon Blade
		[GetAreaInfo(4284)]		= 47,						-- Nesingwary Base Camp/Ironforge
		[GetAreaInfo(4286)]		= 1105,						-- The Bones of Nozronn/The Oracles
		[GetAreaInfo(4287)]		= 1104,						-- Kartak's Hold/Frenzyheart Tribe
		[GetAreaInfo(4288)]		= 1105,						-- Sparktouched Haven/The Oracles
		[GetAreaInfo(4291)]		= 1105,						-- Rainspeaker Canopy/The Oracles
		[GetAreaInfo(4292)]		= 1104,						-- Frenzyheart Hill/Frenzyheart Tribe
		[GetAreaInfo(4297)]		= 1105,						-- Mosswalker Village/The Oracles
		[GetAreaInfo(4303)]		= 1104,						-- Hardknuckle Clearing/Frenzyheart Tribe
		[GetAreaInfo(4306)]		= 1105,						-- Mistwhisper Village/The Oracles
		[GetAreaInfo(4308)]		= 1104,						-- Spearborn Encampment/Frenzyheart Tribe
		[GetAreaInfo(4312)]		= 1098,						-- Ebon Watch/Knights of the Ebon Blade
		[GetAreaInfo(4369)]		= 47,						-- Dorian's Outpost/Ironforge
		[GetAreaInfo(4342)]		= 1098,						-- Acherus: The Ebon Hold (Death Knight start)/Knights of the Ebon Blade
		[GetAreaInfo(4392)]		= 1105,						-- The Stormwright's Shelf/The Oracles
		[GetAreaInfo(4429)]		= 1085,						-- Grom'arsh Crash-Site/Warsong Offensive
		[GetAreaInfo(4418)]		= 21,						-- K3/Booty Bay
		[GetAreaInfo(4428)]		= 1126,						-- Frosthold/The Frostborn
		[GetAreaInfo(4441)]		= 1064,						-- Camp Tunka'lo/The Taunka
		[GetAreaInfo(4442)]		= 1068,						-- Brann's Base-Camp/Explorers' League
		[GetAreaInfo(4458)]		= 21,						-- Sparksocket Minefield/Booty Bay
		[GetAreaInfo(4459)]		= 21,						-- Ricket's Folly/Booty Bay
		[GetAreaInfo(4479)]		= GetFactionID(1094, 1124),	-- Winter's Breath Lake/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4485)]		= 1126,						-- The Inventor's Library/The Frostborn
		[GetAreaInfo(4487)]		= 1085,						-- Frostfloe Deep/Warsong Offensive
		[GetAreaInfo(4501)]		= 1106,						-- The Argent Vanguard/Argent Crusade
		[GetAreaInfo(4502)]		= 1126,						-- Mimir's Workshop/The Frostborn
		[GetAreaInfo(4503)]		= GetFactionID(1094, 1124),	-- Ironwall Dam/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4504)]		= 1106,						-- Valley of Echoes/Argent Crusade
		[GetAreaInfo(4505)]		= 1106,						-- The Breach/Argent Crusade
		[GetAreaInfo(4506)]		= 1106,						-- Scourgeholme/Argent Crusade
		[GetAreaInfo(4507)]		= GetFactionID(1037, 1052),	-- The Broken Front/Alliance Vanguard or Horde Expedition
		[GetAreaInfo(4511)]		= GetFactionID(1037, 1052),	-- The Skybreaker/Alliance Vanguard or Horde Expedition
		[GetAreaInfo(4512)]		= GetFactionID(1037, 1052),	-- Orgrim's Hammer/Alliance Vanguard or Horde Expedition
		[GetAreaInfo(4516)]		= 1106,						-- Ironwall Rampart/Argent Crusade
		[GetAreaInfo(4522)]		= 1156,						-- Icecrown Citadel/The Ashen Verdict
		[GetAreaInfo(4536)]		= GetFactionID(1068, 1085),	-- Frosthowl Cavern/Explorers' League or Warsong Offensive
		[GetAreaInfo(4541)]		= 1106,						-- Vanguard Infirmary/Argent Crusade
		[GetAreaInfo(4558)]		= GetFactionID(1094, 1124),	-- Sunreaver's Command/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4559)]		= GetFactionID(1094, 1124),	-- Windrunner's Overlook/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4580)]		= 1106,						-- Crusaders' Pinnacle/Argent Crusade
		[GetAreaInfo(4593)]		= 1106,						-- The Pit of Fiends/Argent Crusade
		[GetAreaInfo(4616)]		= GetFactionID(1094, 1124),	-- Sunreaver's Sanctuary/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4646)]		= GetFactionID(1094, 1124),	-- Ashwood Lake/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4658)]		= 1106,						-- Argent Tournament Grounds/Argent Crusade
		[GetAreaInfo(4666)]		= GetFactionID(1094, 1124),	-- Sunreaver Pavilion/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4667)]		= GetFactionID(1094, 1124),	-- Silver Covenant Pavilion/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4669)]		= 1106,						-- The Ring of Champions/Argent Crusade
		[GetAreaInfo(4670)]		= 1106,						-- The Aspirants' Ring/Argent Crusade
		[GetAreaInfo(4671)]		= 1106,						-- The Argent Valiants' Ring/Argent Crusade
		[GetAreaInfo(4672)]		= GetFactionID(1094, 1124),	-- The Alliance Valiants' Ring/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4673)]		= GetFactionID(1094, 1124),	-- The Horde Valiants' Ring/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4674)]		= 1106,						-- Argent Pavilion/Argent Crusade
		[GetAreaInfo(4676)]		= GetFactionID(1094, 1124),	-- Sunreaver Pavilion (Outside)/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4677)]		= GetFactionID(1094, 1124),	-- Silver Covenant Pavilion (Outside)/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4740)]		= GetFactionID(1094, 1124),	-- The Silver Enclave/The Silver Covenant or The Sunreavers
		[GetAreaInfo(4760)]		= GetFactionID(1094, 1124),	-- The Sea Reaver's Run/The Silver Covenant or The Sunreavers
		[GetAreaInfo(5561)]		= 1090,						-- The Nexus Entrance/Kirin Tor

		--------- Cataclysm ---------
		[GetAreaInfo(3707)]		= 72,						-- Alliance Base/Stormwind
		[GetAreaInfo(3708)]		= 76,						-- Horde Encampment/Orgrimmar
		[GetAreaInfo(3709)]		= 69,						-- Night Elf Village/Darnassus
		[GetAreaInfo(4779)]		= 76,						-- Horde Base Camp/Orgrimmar
		[GetAreaInfo(4846)]		= 81,						-- Camp Una'fe/Thunder Bluff
		[GetAreaInfo(4851)]		= 81,						-- Ruins of Taurajo/Thunder Bluff
		[GetAreaInfo(4856)]		= GetFactionID(47, 81),		-- Bael Modan/Ironforge or Thunder Bluff
		[GetAreaInfo(4857)]		= 1135,						-- Firestone Point/The Earthen Ring
		[GetAreaInfo(4860)]		= 81,						-- Overgrown Camp/Thunder Bluff
		[GetAreaInfo(4872)]		= GetFactionID(47, 81),		-- Bael'dun Keep/Ironforge or Thunder Bluff
		[GetAreaInfo(4912)]		= 76,						-- Warchief's Lookout/Orgrimmar
		[GetAreaInfo(4946)]		= GetFactionID(47, 81),		-- Bael Modan Excavation/Ironforge or Thunder Bluff
		[GetAreaInfo(4947)]		= 47,						-- Twinbraid's Patrol/Ironforge
		[GetAreaInfo(4948)]		= GetFactionID(47, 1133),	-- Frazzlecraz Motherlode/Ironforge or Bilgewater Cartel
		[GetAreaInfo(4949)]		= 81,						-- Spearhead/Thunder Bluff
		[GetAreaInfo(4969)]		= 72,						-- Tranquil Wash/Stormwind
		[GetAreaInfo(4970)]		= 76,						-- Legion's Rest/Orgrimmar
		[GetAreaInfo(4975)]		= 76,						-- Tenebrous Cavern/Orgrimmar
		[GetAreaInfo(4976)]		= 72,						-- Darkbreak Cove/Stormwind
		[GetAreaInfo(4986)]		= 81,						-- Vendetta Point/Thunder Bluff
		[GetAreaInfo(5050)]		= 1204,						-- Firelands Hatchery (The Hatchery)/Avengers of Hyjal
		[GetAreaInfo(5064)]		= 1204,						-- Firelands Forgeworks (The Anvil of Flame)/Avengers of Hyjal
		[GetAreaInfo(5067)]		= 1204,						-- The Inner Spire/Avengers of Hyjal
		[GetAreaInfo(5099)]		= 1204,						-- The Crucible of Flame (The Crucible)/Avengers of Hyjal
		[GetAreaInfo(5137)]		= 1133,						-- The Krazzworks/Bilgewater Cartel
		[GetAreaInfo(5140)]		= GetFactionID(72, 1133),	-- Highbank/Stormwind or Bilgewater Cartel
		[GetAreaInfo(5155)]		= 1091,						-- Vermillion Redoubt/The Wyrmrest Accord
		[GetAreaInfo(5248)]		= 72,						-- Voldrin's Hold/Stormwind
		[GetAreaInfo(5249)]		= 76,						-- Stygian Bounty/Orgrimmar
		[GetAreaInfo(5293)]		= 1171,						-- Crumbling Depths/Therazane
		[GetAreaInfo(5294)]		= 1171,						-- Shuddering Spires/Therazane
		[GetAreaInfo(5295)]		= 1171,						-- The Pale Roost/Therazane
		[GetAreaInfo(5296)]		= 1171,						-- Therazane's Throne/Therazane
		[GetAreaInfo(5297)]		= 1171,						-- Halcyon Egress/Therazane
		[GetAreaInfo(5298)]		= 1171,						-- Twilight Precipice/Therazane
		[GetAreaInfo(5299)]		= 1171,						-- Lorthuna's Gate/Therazane
		[GetAreaInfo(5300)]		= 1171,						-- Verlok Stand/Therazane
		[GetAreaInfo(5302)]		= 1171,						-- Crimson Expanse/Therazane
		[GetAreaInfo(5338)]		= 1171,						-- Fungal Deep/Therazane
		[GetAreaInfo(5408)]		= 1068,						-- Explorer's League Digsite/Explorer's League
		[GetAreaInfo(5437)]		= 1135,						-- Ring of the Elements/The Earthen Ring
		[GetAreaInfo(5438)]		= 1135,						-- The Maw of Madness/The Earthen Ring
		[GetAreaInfo(5439)]		= 1135,						-- Ruins of Drakgor/The Earthen Ring
		[GetAreaInfo(5461)]		= GetFactionID(72, 1133),	-- Dragonmaw Pass/Stormwind or Bilgewater Cartel
		[GetAreaInfo(5462)]		= GetFactionID(72, 1133),	-- Wyrms' Bend/Stormwind or Bilgewater Cartel
		[GetAreaInfo(5470)]		= GetFactionID(72, 1133),	-- Victor's Point/Stormwind or Bilgewater Cartel
		[GetAreaInfo(5595)]		= 1091,						-- Obsidian Forest/The Wyrmrest Accord
		[GetAreaInfo(5660)]		= 1091,						-- Obsidian Lair/The Wyrmrest Accord
		[GetAreaInfo(5664)]		= 1135,						-- Iso'rath/The Earthen Ring

		--------- MoP ---------
		[GetAreaInfo(4081)]		= 81,						-- The Dawnchaser/Thunder Bluff
		[GetAreaInfo(5876)]		= 1271,						-- Serpent's Heart/Order of the Cloud Serpent
		[GetAreaInfo(5931)]		= 1271,						-- The Arboretum/Order of the Cloud Serpent
		[GetAreaInfo(5974)]		= 1341,						-- Jade Temple Grounds/The August Celestials
		[GetAreaInfo(5975)]		= 1341,						-- Temple of the Jade Serpent/The August Celestials
		[GetAreaInfo(5976)]		= 1270,						-- Gate of the Setting Sun/Shado-Pan
		[GetAreaInfo(6012)]		= 1271,						-- Windward Isle/Order of the Cloud Serpent
		[GetAreaInfo(6013)]		= GetFactionID(1376, 1375),	-- The Dawnchaser (Retreat)/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6016)]		= 69,						-- Sentinel Basecamp/Darnassus
		[GetAreaInfo(6022)]		= 1271,						-- Mistveil Sea/Order of the Cloud Serpent
		[GetAreaInfo(6025)]		= 47,						-- Nesingwary's Safari/Ironforge
		[GetAreaInfo(6048)]		= 1341,						-- Temple of the Red Crane/The August Celestials
		[GetAreaInfo(6059)]		= GetFactionID(1353, 1352),	-- Binan Village/Tushui Pandaren or Huojin Pandaren
		[GetAreaInfo(6080)]		= 1271,						-- Serpent's Overlook/Order of the Cloud Serpent
		[GetAreaInfo(6117)]		= 1341,						-- Fountain of the Everseeing/The August Celestials
		[GetAreaInfo(6118)]		= 1341,						-- The Scrollkeeper's Sanctum/The August Celestials
		[GetAreaInfo(6119)]		= 1341,						-- Terrace of the Twin Dragons/The August Celestials
		[GetAreaInfo(6120)]		= 1341,						-- The Heart of Jade/The August Celestials
		[GetAreaInfo(6143)]		= 1341,						-- Mogu'shan Palace1/The August Celestials
		[GetAreaInfo(6155)]		= 1341,						-- Cradle of Chi-Ji/The August Celestials
		[GetAreaInfo(6160)]		= 1341,						-- Angkhal Pavilion/The August Celestials
		[GetAreaInfo(6161)]		= 1341,						-- Pedestal of Hope/The August Celestials
		[GetAreaInfo(6162)]		= 1341,						-- Dome Balrissa/The August Celestials
		[GetAreaInfo(6174)]		= 1341,						-- Temple of the White Tiger/The August Celestials
		[GetAreaInfo(6182)]		= 1341,						-- Mogu'shan Palace2/The August Celestials
		[GetAreaInfo(6213)]		= 1341,						-- Niuzao Temple/The August Celestials
		[GetAreaInfo(6295)]		= 1345,						-- Seat of Knowledge/The Lorewalkers
		[GetAreaInfo(6368)]		= 1302,						-- Soggy's Gamble/The Anglers
		[GetAreaInfo(6371)]		= GetFactionID(1376, 1375),	-- The Southern Isles/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6393)]		= 1270,						-- Serpent's Spine (Valley of the Four Winds)/Shado-Pan
		[GetAreaInfo(6394)]		= 1270,						-- Serpent's Spine (Vale of Eternal Blossoms)/Shado-Pan
		[GetAreaInfo(6395)]		= 1270,						-- Serpent's Spine (Kun-Lai Summit)/Shado-Pan
		[GetAreaInfo(6401)]		= 1302,						-- Shelf of Mazu/The Anglers
		[GetAreaInfo(6402)]		= 1302,						-- Wreck of the Mist-Hopper/The Anglers
		[GetAreaInfo(6433)]		= 1302,						-- Lonesome Cove/The Anglers
		[GetAreaInfo(6482)]		= GetSingleFactionID("Alliance", 1341),				-- The Summer Terrace/The August Celestials
		[GetAreaInfo(6498)]		= 1341,						-- Gate of the August Celestials/The August Celestials
		[GetAreaInfo(6512)]		= 1271,						-- The Widow's Wail/Order of the Cloud Serpent
		[GetAreaInfo(6513)]		= 1271,						-- Oona Kagu/Order of the Cloud Serpent
		[GetAreaInfo(6533)]		= 72,						-- The Skyfire (Jade Forest)/Stormwind
		[GetAreaInfo(6560)]		= GetSingleFactionID("Horde", 1341),				-- The Golden Terrace/The August Celestials
		[GetAreaInfo(6566)]		= GetFactionID(1376, 1375),	-- Domination Point/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6595)]		= GetFactionID(1376, 1375),	-- The Skyfire/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6596)]		= GetFactionID(1376, 1375),	-- Lion's Landing/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6597)]		= GetFactionID(1376, 1375),	-- Sparkrocket Outpost/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6600)]		= GetFactionID(1376, 1375),	-- Blacksand Spillway/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6601)]		= GetFactionID(1376, 1375),	-- Bilgewater Beach/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6602)]		= GetFactionID(1376, 1375),	-- The Boiling Crustacean/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6604)]		= GetFactionID(1376, 1375),	-- Quickchop's Lumber Farm/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6609)]		= GetFactionID(1376, 1375),	-- Ruins of Ogudei/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6643)]		= GetFactionID(1376, 1375),	-- Lion's Landing (Scenario)/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6644)]		= GetFactionID(1376, 1375),	-- Domination Point (Horde)/Operation: Shieldwall or Dominance Offensive
		[GetAreaInfo(6701)]		= 54,						-- Beeble's Wreck/Gnomeregan
		[GetAreaInfo(6702)]		= 1133,						-- Bozzle's Wreck/Bilgewater Cartel
		[GetAreaInfo(6771)]		= 1341,						-- Celestial Tournament/The August Celestials

		---------- WoD ----------
		[GetAreaInfo(6926)]		= 1515,						-- Veil Shadar/Arakkoa Outcasts
		[GetAreaInfo(6933)]		= 1515,						-- Terokkar Refuge/Arakkoa Outcasts
		[GetAreaInfo(6999)]		= 946,						-- Admiral Taylor's Garrison/Honor Hold
		[GetAreaInfo(7032)]		= 1710,						-- Garden of K'ure/Sha'tari Defense
		[GetAreaInfo(7060)]		= 47,						-- Hemet's Happy Hunting Grounds/Ironforge
		[GetAreaInfo(7061)]		= 930,						-- Yrel's Watch/Exodar
		[GetAreaInfo(7063)]		= 81,						-- Riverside Post/Thunder Bluff
		[GetAreaInfo(7072)]		= 889,						-- Ancestral Grounds/Warsong Outriders
		[GetAreaInfo(7074)]		= 930,						-- Telaar/Exodar
		[GetAreaInfo(7081)]		= 930,						-- Telaari Station/Exodar
		[GetAreaInfo(7139)]		= 1135,						-- Elemental Plateau/The Earthen Ring
		[GetAreaInfo(7141)]		= 1710,						-- Arch of Sha'tar/Sha'tari Defense
		[GetAreaInfo(7156)]		= 889,						-- Anguish Point/Warsong Outriders
		[GetAreaInfo(7162)]		= 1711,						-- Pinchwhistle Gearworks/Steamwheedle Preservation Society
		[GetAreaInfo(7163)]		= 1711,						-- Pinchwhistle Point/Steamwheedle Preservation Society
		[GetAreaInfo(7164)]		= 1711,						-- Wreck of the Mother Lode/Steamwheedle Preservation Society
		[GetAreaInfo(7197)]		= 76,						-- Axefall/Orgrimmar
		[GetAreaInfo(7198)]		= 72,						-- Southport/Stormwind
		[GetAreaInfo(7208)]		= 946,						-- The Briny Barnacle/Honor Hold
		[GetAreaInfo(7222)]		= 946,						-- Admiral Taylor's Farm/Honor Hold
		[GetAreaInfo(7266)]		= 1711,						-- The Rusty Cylinder/Steamwheedle Preservation Society
		[GetAreaInfo(7269)]		= 1710,						-- Spire of Light/Sha'tari Defense
		[GetAreaInfo(7295)]		= 930,						-- Telaari Tankworks/Exodar
		[GetAreaInfo(7377)]		= 2135,						-- Time-Lost Glade/Chromie
		[GetAreaInfo(7389)]		= 1710,						-- Bladefury's Command/Sha'tari Defense
		[GetAreaInfo(7391)]		= 1710,						-- The Path of Glory/Sha'tari Defense
		[GetAreaInfo(7392)]		= 1731,						-- Pillars of Fate/Council of Exarchs
		[GetAreaInfo(7417)]		= 1710,						-- Sha'tari Anchorage/Sha'tari Defense
		[GetAreaInfo(7418)]		= 1710,						-- Shattrath Port Authority/Sha'tari Defense
		[GetAreaInfo(7419)]		= 1710,						-- Beacon of Sha'tar/Sha'tari Defense
		[GetAreaInfo(7420)]		= 1710,						-- Shattrath City/Sha'tari Defense
		[GetAreaInfo(7421)]		= 1710,						-- Shattrath Commons/Sha'tari Defense
		[GetAreaInfo(7422)]		= 1710,						-- Sha'tari Market District/Sha'tari Defense
		[GetAreaInfo(7433)]		= 930,						-- Telaari Worksite/Exodar
		[GetAreaInfo(7453)]		= 1710,						-- Sha'tar Way Station/Sha'tari Defense
		[GetAreaInfo(7454)]		= 1710,						-- Sha'tari Skymesa/Sha'tari Defense
		[GetAreaInfo(7455)]		= 1710,						-- Shattrath Residential District/Sha'tari Defense
		[GetAreaInfo(7480)]		= 1710,						-- Shattrath Overlook/Sha'tari Defense
		[GetAreaInfo(7484)]		= 72,						-- Southport Worksite/Stormwind
		[GetAreaInfo(7536)]		= 1515,						-- Aktar's Post/Arakkoa Outcasts
		[GetAreaInfo(7641)]		= 1850,						-- Fang'rila/The Saberstalkers
		[GetAreaInfo(7655)]		= GetFactionID(1710, 1708), -- Rangari Refuge/Sha'tari Defense or Laughing Skull Orcs
		[GetAreaInfo(7739)]		= 1850,						-- Blackfang Challenge Arena/The Saberstalkers

		--------- Legion ---------
		[GetAreaInfo(7342)]		= 68,						-- Queen's Reprisal/Undercity
		[GetAreaInfo(7542)]		= GetFactionID(1134, 68),	-- Weeping Bluffs/Gilneas or Undercity
		[GetAreaInfo(7555)]		= GetFactionID(1134, 68),	-- Skyfire Triage Camp/Gilneas or Undercity
		[GetAreaInfo(7563)]		= GetFactionID(1134, 68),	-- Cove of Nashal/Gilneas or Undercity
		[GetAreaInfo(7564)]		= GetFactionID(1134, 68),	-- Forsaken Foothold/Gilneas or Undercity
		[GetAreaInfo(7638)]		= 1106,						-- Sanctum of Light (Paladin Class Hall)/Argent Crusade
		[GetAreaInfo(7642)]		= 1134,						-- Bradensbrook/Gilneas
		[GetAreaInfo(7650)]		= 1134,						-- Marl's Stead/Gilneas
		[GetAreaInfo(7679)]		= 1098,						-- Acherus: The Ebon Hold (Broken Isles)/Knights of the Ebon Blade
		[GetAreaInfo(7698)]		= GetFactionID(1134, 68),	-- Dreadwake's Landing/Gilneas or Undercity
		[GetAreaInfo(7699)]		= GetFactionID(1134, 68),	-- Greywatch/Gilneas or Undercity
		[GetAreaInfo(7700)]		= GetFactionID(1134, 68),	-- Ranger's Foothold/Gilneas or Undercity
		[GetAreaInfo(7701)]		= 1134,						-- Crowley's Overlook/Gilneas
		[GetAreaInfo(7733)]		= 47,						-- Nesingwary's Retreat/Ironforge
		[GetAreaInfo(7743)]		= 1098,						-- Acherus: The Ebon Hold (Icecrown)/Knights of the Ebon Blade
		[GetAreaInfo(7773)]		= GetFactionID(1134, 68),	-- Greymane's Offensive/Gilneas or Undercity
		[GetAreaInfo(7778)]		= 1134,						-- Heathrow Manor/Gilneas
		[GetAreaInfo(7779)]		= 1134,						-- Radcliffe Landing/Gilneas
		[GetAreaInfo(7787)]		= 1894,						-- Vault of the Wardens/The Wardens
		[GetAreaInfo(7789)]		= GetFactionID(1134, 68),	-- Bronzeleaf Thicket/Gilneas or Undercity
		[GetAreaInfo(7813)]		= 1948,						-- Skyhold (Warrior)/Valarjar
		[GetAreaInfo(7814)]		= 1894,						-- Vault of the Wardens (Demon Hunter)/The Wardens
		[GetAreaInfo(7836)]		= 1828,						-- Mucksnout Den/Highmountain Tribe
		[GetAreaInfo(7849)]		= GetFactionID(1134, 68),	-- Cullen's Post/Gilneas or Undercity
		[GetAreaInfo(7850)]		= GetFactionID(1134, 68),	-- Lorna's Watch/Gilneas or Undercity
		[GetAreaInfo(7879)]		= 1090,						-- Hall of the Guardian (Mage Class Hall)/Kirin Tor
		[GetAreaInfo(7903)]		= 1341,						-- Temple of Five Dawns (Monk)/The August Celestials
		[GetAreaInfo(7904)]		= 1341,						-- Shrine of Five Dawns (Monk)/The August Celestials
		[GetAreaInfo(8051)]		= GetFactionID(1134, 68),	-- Cordana's Apex/Gilneas or Undercity
		[GetAreaInfo(8078)]		= GetFactionID(1134, 68),	-- Nightwatcher's Perch/Gilneas or Undercity
		[GetAreaInfo(8147)]		= 1894,						-- Isle of the Watchers/The Wardens
		[GetAreaInfo(8237)]		= playerClass == "SHAMAN" and 1135, -- Alluvanon's Delta/The Earthen Ring
		[GetAreaInfo(8270)]		= 1975,						-- Margoss's Retreat/Conjurer Margoss
		[GetAreaInfo(8273)]		= 1134,						-- Thorton's Cottage/Gilneas
		[GetAreaInfo(8284)]		= GetFactionID(69, 911),	-- Illidari Redoubt/Darnassus or Silvermoon City
		[GetAreaInfo(8300)]		= GetFactionID(1134, 68),	-- Theodric's Lodge/Gilneas or Undercity
		[GetAreaInfo(8301)]		= GetFactionID(1134, 68),	-- Greywatch Barracks/Gilneas or Undercity
		[GetAreaInfo(8302)]		= GetFactionID(1134, 68),	-- The Oblivion/Gilneas or Undercity
		[GetAreaInfo(8304)]		= GetFactionID(1134, 68),	-- Starstalker's Point/Gilneas or Undercity
		[GetAreaInfo(8320)]		= GetFactionID(1134, 68),	-- Darkfollow's Spire/Gilneas or Undercity
		[GetAreaInfo(8324)]		= GetFactionID(1134, 68),	-- Whisperwind's Citadel/Gilneas or Undercity
		[GetAreaInfo(8328)]		= GetFactionID(1134, 68),	-- Blackhawk's Bulwark/Gilneas or Undercity
		[GetAreaInfo(8370)]		= 1894,						-- Watchers' Aerie/The Wardens
		[GetAreaInfo(8371)]		= 1894,						-- Wardens' Redoubt/The Wardens
		[GetAreaInfo(8394)]		= 1134,						-- Carlisle Cottage/Gilneas

		--------- BfA ---------
		[GetAreaInfo(8925)]		= 47,						-- Nesingwary's Trek/Ironforge
		[GetAreaInfo(9007)]		= 1068,						-- Explorer's League Campsite/Explorer's League
		[GetAreaInfo(9310)]		= 2164,						-- The Wound/Champions of Azeroth
		[GetAreaInfo(9329)]		= 2163,						-- Seeker's Outpost/Tortollan Seekers
		[GetAreaInfo(9470)]		= 2395,						-- Mildenhall Brewery/Honeyback Hive
		[GetAreaInfo(9473)]		= 69,						-- Staghelm Point/Darnassus
		[GetAreaInfo(9474)]		= 1133,						-- Southwind Village/Bilgewater Cartel
		[GetAreaInfo(9494)]		= 1133,						-- Warfront: The Battle for Stromgarde (Ar'gorok)/Bilgewater Cartel
		[GetAreaInfo(9556)]		= 2163,						-- Tortaka Refuge/Tortollan Seekers
		[GetAreaInfo(9667)]		= 2164,						-- Chamber of Heart (Destroyed)/Champions of Azeroth
		[GetAreaInfo(9693)]		= 2163,						-- Seeker's Vista/Tortollan Seekers
		[GetAreaInfo(9714)]		= 2163,						-- Seeker's Expedition/Tortollan Seekers
		[GetAreaInfo(9735)]		= 1133,						-- Ar'gorok/Bilgewater Cartel
		[GetAreaInfo(9747)]		= 72,						-- Warfront: The Battle for Stromgarde (Refuge Pointe)/Stormwind
		[GetAreaInfo(9749)]		= 70,						-- Dabyrie's Farmstead/The Syndicate
		[GetAreaInfo(9750)]		= 68,						-- Hammerfall/Undercity
		[GetAreaInfo(9755)]		= 47,						-- Thandol Span/Ironforge
		[GetAreaInfo(9756)]		= 68,						-- Thoradin's Wall (Horde Warfront)/Undercity
		[GetAreaInfo(9758)]		= 47,						-- Thoradin's Wall (Alliance Warfront)/Ironforge
		[GetAreaInfo(9759)]		= GetFactionID(47, 68),		-- Thoradin's Wall/Ironforge or Undercity
		[GetAreaInfo(10006)]	= 2163,						-- House of Jol/Tortollan Seekers
		[GetAreaInfo(10504)]	= 2164,						-- Chamber of Heart (rebuilt)/Champions of Azeroth
		[GetAreaInfo(12905)]	= 2164,						-- Chamber of Heart (scenario)/Champions of Azeroth
		[GetAreaInfo(13539)]	= 1098,						-- Acherus: The Ebon Hold (Class Hall)/Knights of the Ebon Blade

		--------- Shadowlands ---------
		[GetAreaInfo(10979)]	= 2413,						-- Darkhaven/Court of Harvesters
		[GetAreaInfo(10995)]	= 2439,						-- Halls of Atonement/The Avowed
		[GetAreaInfo(11533)]	= self.covenantRepID == 3 and 2422 or 2465, -- Tirna Noch/Night Fae or The Wild Hunt
		[GetAreaInfo(12796)]	= 2462,						-- The Stitchyard/Stitchmasters
		[GetAreaInfo(12876)]	= 2410,						-- Seat of the Primus/The Undying Army
		[GetAreaInfo(12901)]	= 2462,						-- Etheric Vault/Stitchmasters
		[GetAreaInfo(13346)]	= 2462,						-- Body Banks/Stitchmasters
		[GetAreaInfo(13347)]	= 2462,						-- Thread House/Stitchmasters
		[GetAreaInfo(13348)]	= 2462,						-- Amputory/Stitchmasters
		[GetAreaInfo(13367)]	= self.covenantRepID == 3 and 2422 or 2465, -- Queen's Conservatory/Night Fae or The Wild Hunt
		[GetAreaInfo(13414)]	= 2413,						-- The Sinposium/Court of Harvesters
		[GetAreaInfo(13515)]	= 2407,						-- Seat of the Archon: Font of Fealty/The Ascended
		[GetAreaInfo(13555)]	= 2462,						-- Sixfold Recluse/Stitchmasters
		[GetAreaInfo(13625)]	= 2472,						-- Keeper's Respite/The Archivists' Codex
		[GetAreaInfo(13652)]	= 2472,						-- Reliquarty of Rememberance/The Archivists' Codex
		[GetAreaInfo(13691)]	= 2472,						-- Chamber of Knowledge/The Archivists' Codex
		[GetAreaInfo(13692)]	= 2472,						-- Chamber of Wisdom/The Archivists' Codex

		--------- Dragonflight ---------
		[GetAreaInfo(13587)]	= 2526,						-- Winterpelt Hollow/Winterpelt Furbolg
		[GetAreaInfo(13710)]	= 2511,						-- Uktulut Backwater/Iskaara Tuskarr
		[GetAreaInfo(13725)]	= 2507,						-- Life Vault Ruins/Dragonscale Expedition
		[GetAreaInfo(13729)]	= 2507,						-- Crumbling Life Archway/Dragonscale Expedition
		[GetAreaInfo(13732)]	= 2507,						-- Dragonscale Basecamp/Dragonscale Expedition
		[GetAreaInfo(13733)]	= 2507,						-- Wild Preserve/Dragonscale Expedition
		[GetAreaInfo(13739)]	= 2517,						-- Obsidian Bulwark/Wrathion
		[GetAreaInfo(13766)]	= 2507,						-- Nelthazan Ruins/Dragonscale Expedition
		[GetAreaInfo(13767)]	= 2507,						-- Emberwatch/Dragonscale Expedition
		[GetAreaInfo(13782)]	= 2520,						-- Nokhudon Hold/Clan Nokhud
		[GetAreaInfo(13801)]	= 2507,						-- Tyrhold/Dragonscale Expedition
		[GetAreaInfo(13802)]	= 2507,						-- Tyrhold Reservoir/Dragonscale Expedition
		[GetAreaInfo(13817)]	= 989,						-- Shifting Sands/Keepers of Time
		[GetAreaInfo(13818)]	= 989,						-- Temporal Conflux/Keepers of Time
		[GetAreaInfo(13822)]	= 989,						-- The Flowing Forest/Keepers of Time
		[GetAreaInfo(13825)]	= 1090,						-- Azure Archives/Kirin Tor
		[GetAreaInfo(13833)]	= 2507,						-- Shiverweb Vale/Dragonscale Expedition
		[GetAreaInfo(13834)]	= 1090,						-- Camp Antonidas/Kirin Tor
		[GetAreaInfo(13835)]	= 2550,						-- Cobalt Assembly/Cobalt Assembly
		[GetAreaInfo(13837)]	= 2511,						-- Iskaara/Iskaara Tuskarr
		[GetAreaInfo(13851)]	= 2550,						-- Cobalt Library/Cobalt Assembly
		[GetAreaInfo(13859)]	= 2507,						-- Camp Nowhere/Dragonscale Expedition
		[GetAreaInfo(13861)]	= 2507,						-- Bluefeather Cliffs/Dragonscale Expedition
		[GetAreaInfo(13886)]	= 2507,						-- Whaler's Nook/Dragonscale Expedition
		[GetAreaInfo(13888)]	= 2507,						-- Three-Falls Lookout/Dragonscale Expedition
		[GetAreaInfo(13895)]	= 2507,						-- Theron's Watch/Dragonscale Expedition
		[GetAreaInfo(13934)]	= 1090,						-- Rhonin's Shield/Kirin Tor
		[GetAreaInfo(13939)]	= 2507,						-- Wingrest Embassy/Dragonscale Expedition
		[GetAreaInfo(13941)]	= 2518,						-- Obsidian Throne/Sabellian
		[GetAreaInfo(14017)]	= 2510,						-- Vakthros Summit/Valdrakken Accord
		[GetAreaInfo(14020)]	= 2510,						-- Lundedane/Valdrakken Accord
		[GetAreaInfo(14079)]	= 2510,						-- Lilac Ramble/Valdrakken Accord
		[GetAreaInfo(14089)]	= 2510,						-- Vakthros/Valdrakken Accord
		[GetAreaInfo(14155)]	= 2511,						-- Morqut Islet/Iskaara Tuskarr
		[GetAreaInfo(14429)]	= 2511,						-- The Filled Bowl/Iskaara Tuskarr
		[GetAreaInfo(14430)]	= 1090,						-- Camp Antonidas: Mage's Rest/Kirin Tor
		[GetAreaInfo(14432)]	= 2507,						-- The Missing Hinge Inn/Dragonscale Expedition
		[GetAreaInfo(14437)]	= 2550,						-- Training Grounds/Cobalt Assembly
		[GetAreaInfo(14438)]	= 2544,						-- Khadin's Cave/Artisan's Consortium

		--------- TWW ---------
		[GetAreaInfo(14758)]	= 2570,						-- Arathi's End/Hallowfall Arathi
		[GetAreaInfo(14774)]	= 1090,						-- Tranquil Strand/Kirin Tor
		[GetAreaInfo(14766)]	= 2570,						-- Wildcamp Ul'ar/Hallowfall Arathi
		[GetAreaInfo(14945)]	= 1090,						-- Faerin's Advance/Kirin Tor
		[GetAreaInfo(14958)]	= 2570,						-- Wildcamp Or'lay/Hallowfall Arathi
		[GetAreaInfo(15329)]	= 2640,						-- Delver's Headquarters/Brann Bronzebeard
		[GetAreaInfo(15526)]	= 2653,						-- Sanity's Rest/The Cartels of Undermine
		[GetAreaInfo(15528)]	= 2653,						-- Gutterville/The Cartels of Undermine

		--------- Midnight ---------
		[GetAreaInfo(15958)]	= 2770,						-- Masters' Perch/Slayer's Duellum
		[GetAreaInfo(15994)]	= 2712,						-- Sunsail Anchorage/Blood Knights
		[GetAreaInfo(15997)]	= 2714,						-- Saltheril's Haven/Saints of the Row
		[GetAreaInfo(16050)]	= 2713,						-- Farstrider Enclave/Farstriders
		[GetAreaInfo(16052)]	= 2696,						-- Amani Pass/Amani Tribe
		[GetAreaInfo(16056)]	= 68,						-- Ruins of Deathholme/Undercity
		[GetAreaInfo(16062)]	= 2713,						-- Lake Elrendar/Farstriders
		[GetAreaInfo(16067)]	= 2713,						-- Farstrider Hold/Farstriders
		[GetAreaInfo(16068)]	= 2711,						-- Stillwhisper Pond/Magisters
		[GetAreaInfo(16071)]	= 2711,						-- Goldenbough Pass/Magisters
		[GetAreaInfo(16087)]	= 2713,						-- Farstrider Square/Farstriders
		[GetAreaInfo(16173)]	= 911,						-- Sunstrider Isle/Silvermoon City
		[GetAreaInfo(16426)]	= 1094,						-- Silverglade/The Silver Covenant
	}
	return subZonesAndFactions
end